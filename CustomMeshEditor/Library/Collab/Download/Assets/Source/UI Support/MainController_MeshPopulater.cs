        ...
        for (int i = 0; i < size; i++)
        {
            for (int j = 0; j < size; j++)
            {
                if (mMeshDropdown.value == 0) //Flat option
                    rotate = Quaternion.Euler(0, 0, -90f); //Initial rotation for flat
                uv[i + j * size] = rotate * new Vector2(i / (float)(size - 1), j / (float)(size - 1)); //Initialize the texture to fit the mesh
            }
        }
